...
/**
 * @param {{ to: string, icon: React.ReactNode, label: string }} props
 */
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[280px_1fr]">
        ...
        <aside className="lg:sticky lg:top-0 lg:h-screen border-b lg:border-b-0 lg:border-r border-slate-800/70 bg-slate-950/60 backdrop-blur">
          <div className="p-4 border-b border-slate-800/70">
            <div className="text-white font-extrabold tracking-wide">RH Manager</div>
            <div className="mt-1 text-xs text-slate-400">
              {entreprise ? (entreprise.nom_commercial || entreprise.nom) : 'Entreprise'}
            </div>
          </div>
          <nav className="p-3 flex flex-col gap-1">
            <SideLink to={base} icon={<FaChartPie />} label="Vue d’ensemble" />
            <SideLink to={`${base}/members`} icon={<FaUsers />} label="Membres" />
            <SideLink to={`${base}/settings`} icon={<FaCog />} label="Paramètres" />
          </nav>
            ...
            <div className="px-4 lg:px-8 py-4 flex items-center justify-between gap-4">
              <div>
                ...
                <p className="text-slate-400 text-sm">
                  {entreprise ? (entreprise.nom_commercial || entreprise.nom) : 'Chargement…'}
                </p>
              </div>
              <a
                href="/logout"
                className="inline-flex items-center justify-center px-4 py-2 rounded-lg bg-red-500/15 text-red-200 hover:bg-red-500/25 transition-colors text-sm font-semibold"
              >
                Déconnexion
              </a>
            </div>
